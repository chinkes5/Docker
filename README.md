...
- **Uptime Kuma**: For uptime monitoring and status pages. [Guide](https://github.com/louislam/uptime-kuma/wiki)
- **Beszel**: For website monitoring. [Guide](https://beszel.dev/guide/what-is-beszel)
- **Dozzle**: A real-time log viewer for other containers. *Not using as I don't read logs!*
...
3.  Start Beszel and Dozzle using their respective compose files (`Beszel.yaml`, `Dozzle.yaml`) as needed. They will automatically connect to the shared network.

### Uptime Kuma
Once installed, create account, add monitors. In settings, add Notifications. Also in settings, configure Cloudflare Tunnel.

### Beszel
This is a hub and agent monitoring tool. The `Beszel.yaml` is for the hub. Create that, go to new site, create account and get the token and key. Use those values to create agents with `beszel-agent.yaml`. Use the guide to attach alerting system via 'Notifications'. Set thresholds via bell icon for each agent.
